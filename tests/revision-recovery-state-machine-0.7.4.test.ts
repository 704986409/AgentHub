...
const revisionBody = {
  reviewId: 'rev-074', reviewerId: 'human', verdict: 'REQUEST_REVISION', summary: 'revise',
  findings: [{ code: 'FIX', severity: 'error', message: 'fix' }], allowNoChangeCompletion: true,
};

const acceptBody = {
  reviewId: 'accept-074', reviewerId: 'human', verdict: 'ACCEPT', summary: 'ok', findings: [],
...
async function createHarness() {
  const directory = mkdtempSync(join(tmpdir(), 'agenthub-074-'));
  ...
  writeFileSync(join(repositoryRoot, 'README.md'), '# AgentHub 0.7.4 Test\n', 'utf8');
  ...
  const project = projects.create({ id: 'project-074', name: 'Revision Recovery' });
  const agent = agents.createAgent({
    id: 'agent-074', projectId: project.id, name: 'RevisionWorker', provider: 'fake', model: 'fake-model',
...
async function setupStartedPlan(h: Awaited<ReturnType<typeof createHarness>>) {
  ...
  const plan = h.planLifecycle.createPlan({
    ...
    tasks: [{
      clientId: 'task-074', parentClientId: null, title: 'Revision Task', description: 'revise',
  ...
  const startRes = await fetch(`${h.baseUrl}/api/v1/plans/${approved.planId}/start`, {
    method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'start-074' },
...
async function reopen(h: Awaited<ReturnType<typeof createHarness>>, registerProvider: boolean) {
  ...
  const address = await server.start();
  return {
    database, tasks, assignmentRepo, agents, reviews, provider, pool, server,
    baseUrl: `http://${address.host}:${String(address.port)}`,
    async close() {
      await server.stop().catch(() => undefined);
...
describe('0.7.4 revision recovery state machine', { timeout: 240_000 }, () => {
  ...
  it('T1 converges a live revision through review and accept', async () => {
    const h = await createHarness();
    harnesses.push(h);
    const started = await setupStartedPlan(h);
    const revised = await fetch(`${h.baseUrl}/api/v1/reviews/${started.handle}/decision`, {
      method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'rev-074-1' },
    ...
    expect(recoveryRow(h.database, started.runtimeTaskId).stage).toBe('REVIEW_READY');
    ...
    const accepted = await fetch(`${h.baseUrl}/api/v1/reviews/${second}/decision`, {
      method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'accept-074-1' },
    ...
    expect(h.assignmentRepo.list()[0]?.status).toBe(AssignmentStatus.COMPLETED);
    expectPoolClean(h.pool, h.agentId);
  });

  it('T2 keeps TURN_STARTED reconciliation and does not replay the provider', async () => {
    const h = await createHarness();
    harnesses.push(h);
    const started = await setupStartedPlan(h);
    let release!: () => void;
    ...
    const pending = fetch(`${h.baseUrl}/api/v1/reviews/${started.handle}/decision`, {
      method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'rev-074-2' },
      body: JSON.stringify(revisionBody),
    });
    await turnStarted;
    expect(recoveryRow(h.database, started.runtimeTaskId).stage).toBe('TURN_STARTED');
    const calls = h.provider.session.runCalls;
    h.database.close();
    release();
    ...
    try {
      expect(recoveryRow(next.database, started.runtimeTaskId).stage).toBe('TURN_STARTED');
      expect(next.tasks.getTask(started.runtimeTaskId)?.status).toBe(TaskStatus.IMPLEMENTING);
  ...
  it('T3 converges durable COMPLETED, FAILED, and NEEDS_INPUT after restart', async () => {
    for (const outcome of ['COMPLETED', 'FAILED', 'NEEDS_INPUT'] as const) {
      const h = await createHarness();
      harnesses.push(h);
      const started = await setupStartedPlan(h);
      ...
      const crashed = await fetch(`${h.baseUrl}/api/v1/reviews/${started.handle}/decision`, {
        method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': `rev-074-${outcome}` },
        body: JSON.stringify(revisionBody),
      });
      ...
      try {
        if (outcome === 'COMPLETED') {
          ...
          expect(recoveryRow(next.database, started.runtimeTaskId).stage).toBe('REVIEW_READY');
        } else {
          ...
          expect(recoveryRow(next.database, started.runtimeTaskId).stage).toBe('TERMINAL');
          expect(next.agents.getAgent(h.agentId)?.status).toBe(AgentStatus.IDLE);
          expect(next.assignmentRepo.list()[0]?.status).toBe(AssignmentStatus.RELEASED);
        }
        expect(next.provider.session?.runCalls ?? 0).toBe(0);
        expectPoolClean(next.pool, h.agentId);
      } finally {
        await next.close();
      }
    }
  ...
  it('T4 shuts down an owned runtime before converging TURN_COMPLETED', async () => {
    const h = await createHarness();
    harnesses.push(h);
    const started = await setupStartedPlan(h);
    h.failpoint.afterRevisionTurnDurableBeforeReview = () => { throw new Error('CRASH_BEFORE_CONVERGENCE'); };
    const crashed = await fetch(`${h.baseUrl}/api/v1/reviews/${started.handle}/decision`, {
      method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'rev-074-4' },
      body: JSON.stringify(revisionBody),
    });
    expect(crashed.status).toBeGreaterThanOrEqual(500);
    expect(recoveryRow(h.database, started.runtimeTaskId).stage).toBe('TURN_COMPLETED');
    const owned = h.pool.inspectAssignmentOwnership(h.agentId, h.assignmentRepo.list()[0]?.id ?? '');
    expect(owned.state).toBe('OWNED');
    expect(owned.exact).toBe(true);
  ...
  it('T5 rebuilds the same assignment runtime before retiring a recovered review', async () => {
    const h = await createHarness();
    harnesses.push(h);
    const started = await setupStartedPlan(h);
    const assignmentId = h.assignmentRepo.list()[0]?.id;
    const revised = await fetch(`${h.baseUrl}/api/v1/reviews/${started.handle}/decision`, {
      method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'rev-074-5' },
      body: JSON.stringify(revisionBody),
    });
    expect(revised.status).toBe(200);
    const second = (await revised.json() as { data: { reviewHandle: string } }).data.reviewHandle;
    ...
    try {
      ...
      const again = await fetch(`${next.baseUrl}/api/v1/reviews/${second}/decision`, {
        method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'rev-074-5b' },
        body: JSON.stringify(revisionBody),
      });
  ...
  it('T6 leaves the review active when runtime restore fails', async () => {
    const h = await createHarness();
    harnesses.push(h);
    const started = await setupStartedPlan(h);
    const revised = await fetch(`${h.baseUrl}/api/v1/reviews/${started.handle}/decision`, {
      method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'rev-074-6' },
      body: JSON.stringify(revisionBody),
    });
    expect(revised.status).toBe(200);
    const second = (await revised.json() as { data: { reviewHandle: string } }).data.reviewHandle;
    ...
    try {
      ...
      const denied = await fetch(`${next.baseUrl}/api/v1/reviews/${second}/decision`, {
        method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'rev-074-6b' },
        body: JSON.stringify(revisionBody),
      });
      ...
      expect(row.stage).toBe('REVIEW_READY');
      expect(next.provider.session?.runCalls ?? 0).toBe(0);
      expect(() => next.pool.getSnapshot(h.agentId)).toThrow();
    } finally {
      await next.close();
    }
  });
