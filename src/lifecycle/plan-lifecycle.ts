...
export class PlanLifecycleService {
  ...
  public isLeadReferenced(agentId:string):boolean{ for(const i of this.#intakes.values())if(i.leadAgentId===agentId)return true; for(const p of this.#plans.values())if(p.leadAgentId===agentId)return true; return false; }
  ...
  public listReviewAuthority():readonly {planId:string;planTaskId:string;runtimeTaskId:string;reviewPending:boolean}[] {
    ...
    for(const p of this.#plans.values()) for(const link of p.runtimeLinks.values()){
      rows.push({planId:p.planId,planTaskId:link.planTaskId,runtimeTaskId:link.runtimeTaskId,reviewPending:link.reviewPending});
    }
