...
export class CursorExecutableResolutionError extends Error {
  public constructor(
    public readonly code: 'EXECUTABLE_NOT_FOUND',
    message: string,
  ) {
    super(message);
    this.name = 'CursorExecutableResolutionError';
  }
}

export interface CursorExecutableResolverDependencies {
...
export function resolveCursorExecutable(
  ...
  dependencies: CursorExecutableResolverDependencies = {},
): string {
  const exists = dependencies.exists ?? existsSync;
  const realpath = dependencies.realpath ?? realpathSync;
  const explicitPath = isPath(command) ? command : undefined;
  if (explicitPath !== undefined) {
    return resolveExistingPath(explicitPath, exists, realpath, 'Explicit Cursor executable');
  }

  const configured = env.CURSOR_EXECUTABLE?.trim();
  if (configured !== undefined && configured.length > 0) {
    return resolveExistingPath(configured, exists, realpath, 'CURSOR_EXECUTABLE');
  }

  const candidatesToTry = command === 'agent' ? ['agent', 'cursor-agent'] : [command];
  ...
  throw new CursorExecutableResolutionError(
    'EXECUTABLE_NOT_FOUND',
    `Cursor executable was not found on PATH (tried ${candidatesToTry.join(', ')})`,
  );
}
...
function resolveExistingPath(
  ...
  if (!exists(path)) {
    throw new CursorExecutableResolutionError('EXECUTABLE_NOT_FOUND', `${source} was not found: ${path}`);
  }
